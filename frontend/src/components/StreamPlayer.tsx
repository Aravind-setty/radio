...
export default function StreamPlayer() {
    ...
    useEffect(() => {
        if (activeStream && activeStream.streamUrl && audioRef.current) {
            audioRef.current.src = activeStream.streamUrl;
            audioRef.current.play().then(() => setIsPlaying(true)).catch(console.error);
        } else {
            setIsPlaying(false);
            if (activeStream && !activeStream.streamUrl) {
                console.warn('Stream has no URL. Audio disabled.');
            }
        }
    }, [activeStream]);
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-gray-800 p-4 text-white flex items-center justify-between shadow-2xl z-50">
            <div className="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-400">by {activeStream.user.username}</p>
                </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
                ...
                <Button onClick={toggleMute} variant="ghost" size="icon">
                    {muted ? <VolumeX className="text-gray-400" /> : <Volume2 className="text-white" />}
                </Button>
                ...
                <Button
                    ...
                    disabled={!activeStream.streamUrl}
                >
                    {activeStream.streamUrl ? (isPlaying ? 'Pause' : 'Play') : 'No Audio'}
                </Button>

                <Button onClick={() => setActiveStream(null)} variant="ghost" size="icon">
