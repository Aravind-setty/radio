...
export default function Dashboard() {
    ...
    const { streams, fetchStreams, createStream, startStream, stopStream, setActiveStream } = useStreamStore();
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        await createStream({
            ...
            type: 'EXTERNAL',
            streamUrl: newStreamUrl // Now truly optional
        });
            ...
            <div className="flex-1 flex flex-col overflow-hidden transition-all duration-300 mr-0">
                ...
                <header className="flex items-center justify-between p-6 border-b border-gray-800 bg-gray-900/50 backdrop-blur-md">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-sm font-medium text-gray-400">Welcome, {user?.username}</span>
                    ...
                    {showCreateModal && (
                        <div className="mb-8 p-6 bg-gray-900 rounded-xl border border-gray-800 animate-in fade-in slide-in-from-top-4">
                            ...
                            <form onSubmit={handleCreate} className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-gray-400">Stream Title</label>
                                    <Input value={newStreamTitle} onChange={e => setNewStreamTitle(e.target.value)} placeholder="Late Night Jazz..." />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-gray-400">Stream URL (Icecast/HLS) - Optional</label>
                                    <Input value={newStreamUrl} onChange={e => setNewStreamUrl(e.target.value)} placeholder="https://icecast.example.com/stream.mp3" />
                                </div>
                        ...
                        {streams.map(stream => (
                            <div key={stream.id}
                                ...
                                <div className="aspect-video bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center relative">
                                    ...
                                    {stream.isActive ? (
                                        <div className="absolute top-3 left-3 flex gap-2">
                                            <span className="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm animate-pulse">LIVE</span>
                                        ...
                                        <div className="absolute top-3 left-3 flex gap-2">
                                            <span className="bg-gray-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">OFFLINE</span>
                                        </div>
                                    )}
                                ...
                                <div className="p-4">
                                    ...
                                    <p className="text-sm text-gray-400 mt-1">DJ: {stream.user.username}</p>

                                    {/* Control for Owner */}
                                    {user?.id === stream.user.id && (
                                        <div className="mt-4 pt-4 border-t border-gray-800 flex justify-between items-center">
                                            <span className="text-xs text-blue-400 font-medium">Your Stream</span>
                                            {stream.isActive ? (
                                                <Button size="sm" variant="destructive" onClick={() => stopStream(stream.id)} className="h-7 text-xs">
                                                    <StopCircle className="w-3 h-3 mr-1" />
                                                    End
                                                </Button>
                                            ) : (
                                                <Button size="sm" onClick={() => startStream(stream.id)} className="h-7 text-xs bg-green-600 hover:bg-green-700">
                ...
                <div className="w-80 h-full border-l border-gray-800 bg-gray-900 shadow-2xl z-40 transition-all">
                    <div className="h-full flex flex-col">
                        <div className="p-2 flex justify-end">
                            <Button variant="ghost" size="sm" className="text-gray-500" onClick={() => setViewingStreamId(null)}>Close</Button>
                        </div>
